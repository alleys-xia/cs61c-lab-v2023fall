...
int main() {
  ...
  // Hint: how many bytes do we need to store this string?
    char* hello_str = malloc(sizeof(char)*(strlen("hello")+1));
  ...
  char* world_str = malloc(sizeof(char)*(strlen("world")+1));
  ...
  char* hello_world_str = malloc(sizeof(char)*(strlen("hello world")+1));
  ...
  // 终止符'\0'在57行中也被复制进hello_world_str中，因此需要替换掉
  hello_world_str[5] = ' ';
